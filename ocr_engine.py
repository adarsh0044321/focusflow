...
class OCREngine:
    ...
    def _preprocess(self, image: Image.Image) -> Image.Image:
        ...
        if self.config.get("ocr_grayscale", True):
            img = img.convert("L")
            self.logger.debug("Preprocessing: grayscale applied.")

        # 2. Contrast enhancement
        if self.config.get("ocr_contrast", True):
        ...
        if self.config.get("ocr_threshold", True):
    ...
    def extract_text(self, image: Image.Image) -> Tuple[str, Optional[str]]:
        ...
        tmp_in = os.path.join(temp_dir, "ocr_input.png")
        tmp_out_base = os.path.join(temp_dir, "ocr_output")
